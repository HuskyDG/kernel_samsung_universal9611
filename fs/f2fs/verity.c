				      ...
				      size_t buf_size)
{
	...
	if (res != sizeof(dloc) || dloc.version != cpu_to_le32(1)) {
		f2fs_msg(F2FS_I_SB(inode)->sb, KERN_WARNING, "unknown verity xattr format");
		return -EINVAL;
	}
	...
	if (pos + size < pos || pos + size > inode->i_sb->s_maxbytes ||
		...
		f2fs_msg(F2FS_I_SB(inode)->sb, KERN_WARNING, "invalid verity xattr");
		return -EFSCORRUPTED;
	}
	if (buf_size) {
		if (size > buf_size)
